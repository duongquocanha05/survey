...
`timestamp`, `response_id`, `survey_version`, `status`, các mã câu hỏi mới từ `S1`, `D1`... `YD2`, `Feedback`, và cuối cùng là `email`. Mã câu hỏi đã được đánh lại liên tục sau khi xóa câu trong bản cũ.
...
4. Trong danh sách hàm ở thanh công cụ, chọn `authorizeDeployment`, bấm **Run**, rồi cấp quyền truy cập Sheet cho tài khoản triển khai. Hàm này tạo tab `Responses_v2` cùng cột `email`.
